...
class TLRStatusController {
    ...
    private static final String TAG = "TLRStatusController";
     ...
     * 刷新状态机
     */
    private TLRLinearLayout.RefreshStatus mRefreshStatus = TLRLinearLayout.RefreshStatus.IDLE;
    /**
     * 加载状态机
     */
    private TLRLinearLayout.LoadStatus mLoadStatus = TLRLinearLayout.LoadStatus.IDLE;
    ...
    TLRStatusController(TLRCalculator calculator, Context context, AttributeSet attrs) {
        ...
        if (array == null) {
            Log.e(TAG, "initAttrs array is null");
            return;
        }
        try {
    ...
    public void calculateMoveRefreshStatus(boolean moveDown) {
        ...
        //y 方向移动正值,切Load为初始状态
        if (totalOffsetY > 0 && mLoadStatus == TLRLinearLayout.LoadStatus.IDLE) {
            if (moveDown) {//view向下运动
                if (mRefreshStatus == TLRLinearLayout.RefreshStatus.IDLE && !isRefreshing) {
                    notifyRefreshStatusChanged(TLRLinearLayout.RefreshStatus.PULL_DOWN);
                }
                if (totalOffsetY >= mCalculator.refreshThresholdHeight && mRefreshStatus == TLRLinearLayout.RefreshStatus.PULL_DOWN) {
                    notifyRefreshStatusChanged(TLRLinearLayout.RefreshStatus.RELEASE_REFRESH);
                    if (isAutoRefreshing) {
                        notifyRefreshStatusChanged(TLRLinearLayout.RefreshStatus.REFRESHING);
                        notifyRefreshStatusChanged(TLRLinearLayout.RefreshStatus.IDLE);
                    }
                }
            } else {//view向上运动
                if (totalOffsetY < mCalculator.refreshThresholdHeight && mRefreshStatus == TLRLinearLayout.RefreshStatus.RELEASE_REFRESH) {
                    notifyRefreshStatusChanged(TLRLinearLayout.RefreshStatus.PULL_DOWN);
                }
    ...
    private void notifyRefreshStatusChanged(TLRLinearLayout.RefreshStatus status) {
    ...
    public void setAutoRefreshing(boolean autoRefreshing) {
        Log.v(TAG, "setAutoRefreshing:" + autoRefreshing);
